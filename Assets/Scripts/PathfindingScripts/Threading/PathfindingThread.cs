            ...
            while (Run)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        watch.Reset();
                        watch.Start();
                        var result = Pathfinder.Run(request.StartX, request.StartY, request.EndX, request.EndY, request.Config, manager.Provider, request.ExistingList, out l);
                        watch.Stop();
                        LatestTime = watch.ElapsedMilliseconds;
                        TimeThisSecond += LatestTime;

                        PathfindingResolution res = request.Resolution;
                        res.Path = manager.Provider.TranslatePath(l);
                        res.Result = result;
                        res.Callback = request.ReturnEvent;
                        res.Id = request.Id;
                        res.FramesToLive = Constants.FRAMES_TO_LIVE;

                        if (res.Path == null)
                        {
                            throw new Exception($"Path from {request.StartX}, {request.StartY} to {request.EndX}, {request.EndY} failed with result {result}");
                        }

                        //Debug.Log($"Thread number {ThreadNumber} adding ID {request.Id} with path of length {res.Path.Count}");
                        // Got the results, now enqueue them to be given back to the main thread. The method automatically locks.         
                        manager.AddResponse(res, request);
                    }
                }
                catch (Exception e)
                {
