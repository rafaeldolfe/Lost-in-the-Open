...
public class ProgramLog : MonoBehaviour
{
    ...
    public void LogMoveActor(GameObject invoker, List<object> parameters, int x, int z, int tx, int tz)
    {
        Event ev = new Event(invoker.name, String.Format("Move {0} from tile ({1},{2}) to ({3},{4})", invoker.name, x, z, tx, tz), parameters);

        Log(ev);
    }
    public void LogAttack(GameObject invoker, List<object> parameters, int x, int z, int tx, int tz)
    {
        Event ev = new Event(invoker.name, String.Format("Attack from tile ({0},{1}) to ({2},{3})", x, z, tx, tz), parameters);

        Log(ev);
    }
    public void LogDeath(GameObject invoker, List<object> parameters, int x, int z, int tx, int tz)
    {
        Event ev = new Event(invoker.name, String.Format("{0} at ({1},{2}) died", invoker.name, x, z), parameters);

        Log(ev);
    }
    public void LogOfferEndTurn(GameObject invoker, List<object> parameters, int x, int z, int tx, int tz)
    {
        Event ev = new Event(invoker.name, String.Format("Player prompted to end turn"), parameters);

        Log(ev);
    }
    public void LogEndTurn(GameObject invoker, List<object> parameters, int x, int z, int tx, int tz)
    {
        Event ev = new Event(invoker.name, String.Format("Player ended turn"), parameters);

        Log(ev);
    }
    public void LogBeginAITurn(GameObject invoker, List<object> parameters, int x, int z, int tx, int tz)
    {
        Event ev = new Event(invoker.name, String.Format("AI began turn"), parameters);

        Log(ev);
    }
    public void LogEndAITurn(GameObject invoker, List<object> parameters, int x, int z, int tx, int tz)
    {
        Event ev = new Event(invoker.name, String.Format("AI ended turn"), parameters);
